...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
					...
					onRefresh: options.onRefresh,                       //对表格进行刷新时触发
        ...
        operate: {
			...
			// 保存信息 刷新表格
			saveFormData: function(url, data, callback) {
				var config = {
					url: url,
					type: "post",
					processData: false,
            ...
            successTabCallback: function(result) {
                ...
                $.modal.closeLoading();
            			...
            			// 修改信息
            			conmmonOperate: function(id,ititle) {
            				table.set();
            				if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
            					var row = $("#" + table.options.id).bootstrapTreeTable('getSelections')[0];
            					if ($.common.isEmpty(row)) {
            					...
            					var url = table.options.operateUrl.replace("{id}", row[table.options.uniqueId]);
            					$.modal.open(ititle + table.options.modalName, url);
            				} else {
            					$.modal.open(ititle + table.options.modalName, $.operate.conmmonOperateUrl(id));
            				}
            			},
            			// 修改信息，以tab页展现
            			conmmonOperateTab: function(id,ititle) {
            				table.set();
            				$.modal.openTab(ititle + table.options.modalName, $.operate.conmmonOperateUrl(id));
            			},
            			// 修改信息 全屏
            			conmmonOperateFull: function(id,ititle) {
            				table.set();
            				var url = "/404.html";
            				if ($.common.isNotEmpty(id)) {
            					url = table.options.operateUrl.replace("{id}", id);
            				} else {
            					var row = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
            					url = table.options.operateUrl.replace("{id}", row);
            				}
            				$.modal.openFull(ititle + table.options.modalName, url);
            			},
            			// 修改访问地址
            			conmmonOperateUrl: function(id) {
            				var url = "/404.html";
            				if ($.common.isNotEmpty(id)) {
            					url = table.options.operateUrl.replace("{id}", id);
            				} else {
